...
- **`BitcoinCoreTool.getTransactionWithInputs`** — default `verbose=2` for inline prevouts on Core ≥ 25.0 (zero extra RPCs); fall back to a batched `verbose=1` N-prevout walk when `bitcoinCoreVerbose: 1` is set explicitly (matches Python's `impl/utxo/tools/bitcoin_core.py`). New adapter constructor option: `bitcoinCoreVerbose?: 1 | 2` (default `2`).
