...
- **`BitcoinCoreTool.getTransactionWithInputs`** — default `verbose=1` with a batched N-prevout walk (matches Python's `impl/utxo/tools/bitcoin_core.py:453` cost profile and works on every Core version). Opt in to `bitcoinCoreVerbose: 2` on Core ≥ 25.0 nodes to save the extra round-trips by parsing inline `vin[*].prevout`. New adapter constructor option: `bitcoinCoreVerbose?: 1 | 2` (default `1`). Setting `2` on Core < 25 will fail — the older RPC returns the `verbose=1` shape and the adapter cannot find the expected `prevout`; the tradeoff is explicit and lives with the caller who opts in.
