...
| `CHAIN_ID_TO_RANGO_NAME` + accessors | [rango_chain_names.ts](../rango_chain_names.ts) | Bidirectional map between omnichain chain ids and Rango's canonical uppercase `blockchains[].name`. Consumers (rango-intents price feed, gasless per-chain config, depositron proof validators) import from here instead of maintaining local copies. `NOT_ON_RANGO` set records explicit "not on Rango" decisions; the coverage test forces a decision on every new chain id |
